...
namespace assignment1.Data
{
    public class DBConnector : MainData
    {
        ...
        #region Reviews
        public bool AddReview(ReviewModel _review)
        {
            var model = GetDataModel("ReviewType");
            model.Rows.Add(_review.ToDataRow(model));
            return this.NonQueryExecuteSQL("[brb_CRUD_review]", new Hashtable() { { "@table", model }, { "@type", CRUD.Create } }) == 1;
        }

        public IEnumerable<ReviewModel> GetReviewByUser(int _user_id)
        {
            DataTable info = this.GetSQLData("brb_get_reviews_by_user", new Hashtable() { { "@user_id", _user_id } });
            foreach (DataRow row in info.Rows)
                yield return new ReviewModel(row);
        }
        #endregion
